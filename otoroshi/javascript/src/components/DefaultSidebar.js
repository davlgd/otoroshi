...
export function DefaultSidebar(props) {
  ...
  const preferences = (props.env.user_preferences || { preferences: {} })
    .preferences;
  ...
  const removeShortcut = (shortcut) => {
    if (shortcut.custom) {
      ...
      const newShortcuts = shortcuts.filter(
        (f) => isObject(f) || !f.includes(shortcut.title)
      );
      writeStorage(newShortcuts);
    }
  };
  ...
  if (
    window.location.pathname.startsWith(
      "/bo/dashboard/extensions/workflows/"
    ) &&
  ...
  return (
    <>
      ...
      {openedSidebar && !onRouteTab && (
        <ul
          className="nav flex-column nav-sidebar me-2"
          style={{ marginTop: 20 }}
        >
        ...
        >
          {features
            ...
            .map(({ title, link, icon, tag }) => {
              const alreadyInShortcuts = !!shortcuts.find(
                (s) => s === title.toLowerCase()
              );
              if (link.indexOf("http") === 0) {
                const iconTitle = description
                  ? `${title} - ${description}`
                  : title;
                return (
                  <a
                    ...
                    onClick={(e) => {
                      if (!addShortcutButton) {
                        writeStorage([
                          ...new Set([...shortcuts, title.toLowerCase()]),
                        ]);
                      }
                    }}
                  >
